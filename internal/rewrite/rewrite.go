...
// SetDepVersion rewrites the version in a dep(name = "…", version = "…") declaration
...
func SetDepVersion(path, moduleName, newVersion string) error {
	...
	// Pattern: dep( ... name = "moduleName" ... version = "old" ...)
	// We match lines containing the module name and replace the version value.
	pat := regexp.MustCompile(`(dep\s*\([^)]*name\s*=\s*"` + regexp.QuoteMeta(moduleName) + `"[^)]*version\s*=\s*")[^"]*(")`)
	if !pat.Match(data) {
		return fmt.Errorf("rewrite: dep for module %q not found in %s", moduleName, path)
	}

	updated := pat.ReplaceAll(data, []byte(`${1}`+newVersion+`${2}`))
