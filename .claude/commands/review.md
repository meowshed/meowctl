...
Review: **$ARGUMENTS** - the current branch against `rust-rewrite` if no target
is given.
...
3. **Parity.** For anything touching a format, a builtin, or a command output,
   check it against `v0.1.0`. A lock file whose keys reorder, a builtin whose
   error text changes, a command whose exit code shifts: each breaks a user who
   has both binaries on the same machine.

4. **Failure paths.** Every error branch the spec specifies: is it implemented,
   and does a test drive it? This is where the Go tree accumulated its defects
   and it is where a review pays for itself.

5. **Test quality.** Would each test fail if its requirement were violated? A
   test that asserts the code does what the code does is worse than no test,
   because it reports coverage it does not provide.

6. **Boundaries.** `meowctl-common` imports no workspace crate. `meowctl-tui`
   does not know the engine exists. Nothing below `meowctl-cli` constructs an
   effect or branches on a dry-run flag. A change that crosses one of these is
   a finding regardless of how small it is, because these are the boundaries
   the whole rewrite rests on.

7. **Scope.** Behaviour beyond the cited requirements does not belong here.

8. **Conventions.** Commit format, pull request body, and - always - no mention
